...
| Test integrity (v0.2.1) | [`scripts/quality/test_integrity.py`](scripts/quality/test_integrity.py), `quality / test-integrity` | On by default. On pull requests it fails on removed assertions, removed tests, new skip markers or deleted test files, checked per assertion and per test with no netting, unless each file is declared in `.github/test-weakening.md` (Owner-gated by CODEOWNERS) and named in the PR's "Removed or weakened tests or policy" section, which is cross-checked against the diff |
