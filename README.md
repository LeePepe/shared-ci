...
released or Owner-policy-approved. Start at the [candidate entry](ai/README.md)
...
| Test integrity (v0.2.1 candidate) | [`scripts/quality/test_integrity.py`](scripts/quality/test_integrity.py), `quality / test-integrity` ([contract](docs/test-integrity.md)) | Default-on, independently of changed-layer selection. Detects undeclared test losses against the whole PR; the contract defines loss kinds, ledger/body/CODEOWNERS checks, CLI usage and heuristic limits. Strict policy A remains Owner-pending. |
