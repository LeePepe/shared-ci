...
The selection/integrity features below are **development candidates**, not
released. The Owner approved test changes without Owner approval and index-only
AGENTS; implementation/release review remains. Start at the [candidate entry](ai/README.md)
for status and same-SHA routing. No release or real-consumer adoption evidence
for these features is supplied here.
...
| Test integrity (v0.2.1 candidate) | [`scripts/quality/test_integrity.py`](scripts/quality/test_integrity.py), `quality / test-integrity` ([contract](docs/test-integrity.md)) | Default-on, independently of layer selection. Detects whole-PR test losses and checks the PR explanation; test changes need ordinary AI review, not Owner approval or a ledger. The contract documents the heuristic limits. |
...
1. Pick one reviewed full shared-ci commit SHA. For selection/integrity, first
   satisfy the [candidate upgrade prerequisites](docs/integration-and-migration.md#selection-and-integrity-upgrade);
   a commit's existence on `main` does not establish release approval.
2. Use that SHA everywhere:
   ...
   - `.github/workflows/review.yml`: `codex-review.yml@<SHA>` and
     `kimi-review.yml@<SHA>`;
   - `.github/repo-contract.json`: `shared_ci: <SHA>`; other exact shared-library
     versions go in `dependencies`. AGENTS is only the linked document index.
     Legacy callers without metadata retain their old AGENTS pin.
...
every pin and repository metadata in one PR. The audit rejects pins that are
...
| Diagnose or declare a test loss | [Test integrity](docs/test-integrity.md) |
