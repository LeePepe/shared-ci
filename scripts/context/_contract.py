...
USES = re.compile(r"(?i)^leepepe/shared-ci/([^@\s]+)@([^\s]+)$")
# Same ASCII SemVer grammar as repo-metadata-v1.json, plus v prefix/full SHA.
EXACT_VERSION = re.compile(
    r"(?:[0-9a-f]{40}|v?(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)"
...
def workflow_calls(report: Any, root: pathlib.Path, files: dict) -> list[tuple]:
    """Parsed shared-ci calls: (file, structural location, workflow, ref, inputs).

    All tracked workflows are parsed, including ones with no plain uses line.
    Unsupported syntax/read failures are findings, never an empty safe inventory.
    """
    calls = []
    parser = _frontmatter()
    for path in sorted(files):
        if not re.match(r"^\.github/workflows/[^/]+\.ya?ml$", path):
            continue
        try:
            text = _tracked_text(root, path, files)
            if text is None:
                raise ValueError("workflow must be a tracked readable regular file")
            ...
            for job_id, job in jobs.items():
                if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_-]*", job_id):
                    raise ValueError("job IDs must be unescaped identifiers")
                if not isinstance(job, dict):
                    raise ValueError(f"jobs.{job_id} must be a mapping")
                steps = job.get("steps", [])
                if not isinstance(steps, list):
                    raise ValueError(f"jobs.{job_id}.steps must be a sequence")
                nodes = [(f"jobs.{job_id}", job)] + [
                    (f"jobs.{job_id}.steps.{index}", step) for index, step in enumerate(steps)]
                for location, node in nodes:
                    if not isinstance(node, dict):
                        raise ValueError(f"{location} must be a mapping")
                    if any("\\" in key for key in node):
                        raise ValueError(f"{location}: escaped mapping keys are unsupported")
                    if "uses" not in node:
                        continue
                    uses, inputs = node["uses"], node.get("with", {})
                    if not isinstance(uses, str) or not uses or not isinstance(inputs, dict):
                        raise ValueError(f"{location}: uses must be a string and with a mapping")
                    match = USES.fullmatch(uses)
                    if match:
                        calls.append((path, location, match[1], match[2], inputs))
                    elif uses.lower().startswith("leepepe/shared-ci/"):
                        raise ValueError(f"{location}: malformed shared-ci reference")
        except (ValueError, TypeError, RecursionError) as error:
            report.add("ci", path, f"cannot resolve workflow calls: {error}")
    return calls
...
def _ci(report: _Report, files: dict, refs: list) -> None:
    ...
    for file, location, workflow, _ in refs:
        if (file in ci_files and workflow == ".github/workflows/quality.yml"
                and location != "jobs.quality"):
            report.add("ci", file, f"{location} calls quality.yml; name the job 'quality' so the "
                                   "required check is 'quality / aggregate'")
    for file, location, path, ref in refs:
        if not SHA.fullmatch(ref):
            report.add("ci", f"{file}:{location}", f"{path}@{ref} is not pinned to a full 40-char SHA")
    if len({ref for *_, ref in refs}) > 1:
        report.add("ci", ".github/workflows", f"mixed shared-ci pins: {sorted({r for *_, r in refs})}")


def _review_rules(report: _Report, calls: list, metadata: dict | None) -> None:
    if not metadata:
        return
    for path, location, workflow, _, inputs in calls:
        if (location.count(".") == 1 and workflow in (
                ".github/workflows/codex-review.yml", ".github/workflows/kimi-review.yml")
                and inputs.get("rules-file") != metadata["guide"]):
...
def _valid_owner(owner: str) -> bool:
    # Syntactic admission only; existence, visibility and write access are live checks.
    login = r"[A-Za-z0-9]+(?:-[A-Za-z0-9]+)*"
    if owner.startswith("@"):
        return (len(owner[1:].split("/", 1)[0]) <= 39
                and re.fullmatch(r"@" + login + r"(?:/[A-Za-z0-9][A-Za-z0-9_-]*)?", owner) is not None)
    atom = r"[A-Za-z0-9!$%&'*+/=?^_`{|}~-]+"
    label = r"[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?"
    return re.fullmatch(atom + r"(?:\." + atom + r")*@" + label + r"(?:\." + label + r")+", owner) is not None
            ...
            for parts in rules:
                for owner in parts[1:]:
                    if not _valid_owner(owner):
                        raise ValueError(f"invalid CODEOWNERS owner: {owner!r}")
...
def audit(ctx: Any, root: pathlib.Path) -> list[Any]:
    ...
    calls = workflow_calls(report, root, files)
    refs = [call[:4] for call in calls]
    ...
    _ci(report, files, refs)
    _review_rules(report, calls, metadata)
